...
1. Collect a bounded parent brief: 2–4 sentences of what just changed; the **full** changed-file list; and an authored-only diff, following [generated-file filtering](references/generated-files.md). Include staged + unstaged changes; use `HEAD~1` only when the user asked to review the last commit. Keep excluded generated paths in a separate `generated-omitted:` inventory, never in `truncated:`. Paste at most ~1500 diff lines (~80k characters). If you truncate, name every omitted file in a `truncated:` list and tell the reviewer those files must be re-read with Read — do not treat the paste as the whole change. Do not paste the full chat.
