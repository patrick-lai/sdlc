...
- Trace dynamic values through path, selector, URL, query and serializer boundaries. Confirm client/server compatibility from authored schemas, operations and generation checks. Generated output bodies stay excluded unless a concrete discrepancy needs a narrow lookup.
...
- Inspect build/runtime or pre-merge/post-merge differences only when configuration, dependencies or generator inputs change, using existing generation evidence for generated-output churn.
