...
export const REPORT_DATA_TOKEN = '__BE_REVIEW_REPORT_JSON__'
...
function buildExecutiveSummary({ verdict, counts, qa, featureGate, findings, failedNodes }) {
  ...
  const decision =
    verdict.value === 'blocked'
      ? 'Do not merge yet.'
      : qaBlocksApproval
        ? 'Investigate backend verification before approving.'
        : verdict.value === 'passable'
  ...
  const nextStep =
    verdict.value === 'blocked'
      ...
      : qaBlocksApproval
        ? 'Open the verification section and resolve the FAIL, PARTIAL, or BLOCKED result first.'
        : verdict.value === 'passable'
          ? 'Scan notes and verification, then you can approve.'
  ...
  if (qa.status === 'fresh') {
    ...
    if (outcome === 'PASS') bullets.push('Backend verification passed on this head (H0).')
    else if (outcome === 'FAIL') bullets.push('Backend verification failed on this head (H0).')
    else if (outcome === 'PARTIAL') bullets.push('Backend verification is partial on this head (H0) — not full proof.')
    else if (outcome === 'BLOCKED') bullets.push('Backend verification was blocked on this head (H0).')
    else bullets.push('Backend verification report is fresh for this head (H0); see verification section for outcome.')
  } else if (qa.status === 'not-run') bullets.push('Backend verification was not run for this head.')
  else bullets.push(`Backend verification is ${qa.status} — backend proof cannot be relied on.`)
...
export function buildReportDocument({
  ...
  const gate =
    featureGate ||
    (selected.includes('tests-rollout')
      ? {
          status: 'unverified',
          rationale: 'Rollout reviewer did not produce a valid feature-gate decision.',
          evidence: ['Missing or invalid tests-rollout node.'],
          keys: [],
        }
      : {
          status: 'not-applicable',
          rationale: 'No tests-rollout persona was selected.',
          evidence: ['Changed-file classification did not select tests-rollout.'],
          keys: [],
        })

  const rows = coverage || []
  const gatePath = rows.filter((row) => row.persona === 'tests-rollout')
  ...
  const qaDoc = {
    ...
    reason:
      qa?.reason ||
      qa?.error ||
      (qa?.status === 'not-run' ? 'Verification was not run.' : 'None reported.'),
  }
  ...
  return {
    schema: 'sdlc.be-pr-review.report/v1',
    skill: 'be-pr-review',
...
export function buildReviewReport(input) {
  ...
  const rollout = input.nodeResults?.find((node) => node.persona === 'tests-rollout' && node.status === 'ok')?.value
  const rolloutDecision = rollout?.rolloutRequirement
    ? { ...rollout.rolloutRequirement, keys: rollout.rolloutRequirement.identifiers || [] }
    : null
  const featureGate =
    input.featureGate ||
    rolloutDecision ||
    (input.selected?.includes('tests-rollout')
      ? {
          status: 'unverified',
          rationale: 'Rollout reviewer did not produce a valid rollout decision.',
          evidence: ['Missing or invalid tests-rollout node.'],
          keys: [],
        }
      : {
