...
import { DEFAULT_RUNTIME_POLICY, PERSONA_FACETS, assertOutsideRepo, assertPortableConsent, assertSafeModelId, buildReviewReport, buildRunnerCommand, createSnapshot, classifyRunnerFailure, clip, deriveDecision, discoverRunners, executeRunner, makePlan, nativeSynthesisPrompt, normalizeRuntimePolicy, parseArgs, qaEvidence, qaForPrompt, runGraph, selectPersonas, validateCandidate, validateFanoutEvidence, validateNativeSynthesis, validateSynthesis } from './review-graph.mjs'
...
try {
  ...
  assert.throws(() => createSnapshot({ repoRoot: repo, base: 'HEAD^', output: path.join(temp, 'expired-snapshot') }, { deadlineMs: 100, now: () => 100 }), /deadline exceeded/)
  ...
  const filteredSnapshot = JSON.parse(fs.readFileSync(path.join(temp, 'native-plan/snapshot/snapshot.json')))
  assert.deepEqual(filteredSnapshot.changedFiles, ['Button.tsx', 'codegen.ts', 'query.graphql'])
  ...
  const filteredPatch = fs.readFileSync(path.join(temp, 'native-plan/snapshot/diff.patch'), 'utf8')
