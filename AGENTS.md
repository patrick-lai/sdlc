...
4. For **fe-pr-review** and **be-pr-review**: default to focused concurrent native specialists with complete risk coverage. Preserve immutable evidence, disconfirmation, revision-bound checks and opt-in visual QA. Legacy graph runners and formal artifact contracts are explicit-only compatibility tools; external runners remain consent-gated. Do not make reviewer counts, overlap logs, report formats or unrelated facet checklists a prerequisite for ordinary review.
5. Author shared review behavior in `templates/review-workflow.md` and lenses in `skills/review/references/lenses.md`; `npm run sync:plugins` copies them into all three standalone skill packages. Keep their entrypoints small and self-contained. Canonical generated-file filtering lives in `skills/review/scripts/review-context.mjs` and `skills/review/references/generated-files.md`; sync copies both into FE, BE and second-opinion. Generated outputs are excluded from default model patches, with omission metadata and full change identity retained. Handwritten GraphQL inputs, migrations, lockfiles and test snapshots remain reviewable.
6. For **review**: resolve the target once, preserve dirty-worktree bytes and one shared `H0`, select lenses from behavior and return one verdict. Mixed changes do not run two full reviews. Reuse a host's assigned panel and checks. Learned knowledge may select probes but never proves a finding. Launch independent native specialist scopes up to available host capacity while the coordinator covers integration seams and verifies candidates. Keep failed scopes outstanding until covered or disclosed; no separate synthesis agent on the default path.
